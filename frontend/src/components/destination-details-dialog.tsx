...
/** Weather panel shown in the "Quick planning info" column. */
        ...
        {weather && !loading && (
            ...
            >
              ...
              <div className="mb-3 flex items-center gap-3">
                <span className="text-3xl leading-none" role="img" aria-label={weather.condition}>
                  {weather.emoji}
                </span>
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{weather.condition}</p>
                </div>
              </div>

              {/* Wind + humidity row */}
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center gap-1.5 rounded-xl border border-border/50 bg-background/60 px-3 py-2 text-xs text-muted-foreground">
                  <Wind className="h-3 w-3 shrink-0 text-primary/70" />
                  <span>{weather.windspeedKmh} km/h wind</span>
                </div>
                <div className="flex items-center gap-1.5 rounded-xl border border-border/50 bg-background/60 px-3 py-2 text-xs text-muted-foreground">
                  <Droplets className="h-3 w-3 shrink-0 text-primary/70" />
                    ...
                    <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
                      ...
                      <div className="rounded-[var(--radius-card)] border border-border/70 bg-gradient-to-br from-primary/[0.06] via-white to-secondary/[0.08] p-6 dark:from-primary/10 dark:via-card dark:to-secondary/10">
                        ...
                        <div className="mt-4 grid gap-4 text-sm">
                          {/* Live weather — shown first so it's the most actionable info */}
                          <WeatherPanel
                              lat={dest.latitude}
                              lon={dest.longitude}
                              destName={dest.name}
