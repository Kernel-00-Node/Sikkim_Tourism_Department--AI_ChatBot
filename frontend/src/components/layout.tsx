...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const linkInactive = isHome ? "text-white/60 hover:text-white/90" : "text-muted-foreground hover:text-foreground";
  const linkActiveBg = isHome ? "bg-white/15 border-white/20"       : "bg-primary/10 border-primary/20";
  ...
  return (
      <div className="flex flex-col min-h-[100dvh]">
        ...
        <header className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-500 ${headerBg}`}>
          <div className="container mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
            ...
            <Link href="/" className="flex items-center gap-3 group shrink-0">
              ...
              <span className={`font-serif text-[1.05rem] font-bold tracking-tight drop-shadow-sm transition-colors duration-300 ${txtMain}`}>
                Sikkim Tourism
              </span>
                <span className={`text-[0.58rem] font-semibold tracking-[0.18em] uppercase mt-0.5 transition-colors duration-300 ${txtMuted}`}>
                &amp; Civil Aviation Dept.
              </span>
              </div>
            </Link>

            <div className={`hidden md:flex items-center gap-2 px-3 py-1 rounded-full border backdrop-blur-sm transition-all duration-300 ${badgeCls}`}>
            ...
            <nav className="flex items-center gap-1">
              {navLinks.map(({ href, label, icon: Icon }) => {
                ...
                return (
                    ...
                    >
                      <span className={`absolute inset-0 rounded-lg transition-all duration-200 border ${active ? linkActiveBg : `bg-transparent border-transparent ${linkHoverBg}`}`} />
        ...
        <main className={`flex-1 flex flex-col relative ${isHome ? "" : "pt-16"}`}>{children}</main>
        ...
        <footer className="border-t border-border bg-card/60 backdrop-blur-sm py-5">
          <div className="container mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-[0.68rem] text-muted-foreground tracking-wide">
            <span>© {new Date().getFullYear()} Department of Tourism &amp; Civil Aviation, Government of Sikkim</span>
